...
func init() {
	c.Plugins.Add(&c.Plugin{UName: "discord", Name: "Discord", Author: "Azareal", Init: discordInit, Activate: discordActivate, Deactivate: discordDeactivate, Meta: discordMeta()})
}

func discordMeta() c.PluginMetaData {
	return c.PluginMetaData{
		Hooks: []string{
			"action_end_create_topic",
			"action_end_create_reply",
		},
	}
}
...
func discordInit(pl *c.Plugin) error {
	err := discordValidate()
	if err != nil {
		return err
	}
	pl.AddHook("action_end_create_topic", discordEventTopic)
	pl.AddHook("action_end_create_reply", discordEventReply)
	return nil
}

// A bit of validation to make sure the admin isn't forgetting something or telling Plugin Discord to do something absurd
func discordActivate(pl *c.Plugin) error {
	return discordValidate()
}

func discordDeactivate(pl *c.Plugin) {
	pl.RemoveHook("action_end_create_topic", discordEventTopic)
	pl.RemoveHook("action_end_create_reply", discordEventReply)
}
...
func discordEvent(typ, id int) {
	...
	preURL := "http" + s + "://" + c.Site.URL

	avatar := user.MicroAvatar
